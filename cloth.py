...
# define the ball
ball_radius = 0.2
ball_center = ti.Vector.field(3, ti.f32, (1,))

# use ti.f32 instead of float, will be different?
# the N x N array makes my particles (1-D array) a joke 0.0
...
# init the links (constraints / edges)
...
def step():
    ...
    for i in ti.grouped(x):
        ...
        for d in ti.static(links):
            # clamp the coordinate into the range of 0x0 ~ N-1xN-1
...
while window.running:
    ...
    scene.mesh(vertices, indices=indices, color=(0.2, 0.2, 0.3), two_sided=True)
